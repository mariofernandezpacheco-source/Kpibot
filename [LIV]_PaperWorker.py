# [LIV]_PaperWorker.py — worker.sh con Parquet, MLflow (vía entrenos externos), TSCV thresholds y logging estructurado
import json
import os
import subprocess
import sys
...
def run_daily_prep_once(logger):
    ...
    if bool(getattr(S, "downloader_on_start", False)):
        try:
            downloader_path = project_root / "utils" / "F_download_all_tickers_FOR.py"
    ...
    if bool(getattr(S, "training_on_start", False)):
        try:
            training_path = project_root / "[TRN]_Train.py"
            training_cmd = [
                py,
                "-u",
                str(training_path),
    ...
    def _run_cv_for(ticker: str) -> bool:
        cv_path = project_root / "[RSH]_TimeSeriesCV.py"
        args = [
            py,
            "-u",
            str(cv_path),
