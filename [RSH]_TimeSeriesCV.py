# [RSH]_TimeSeriesCV.py — TSCV con selección robusta de threshold + MLflow + logging estructurado
...
train_mod = importlib.import_module("04_model_training")
ensure_atr14 = train_mod.ensure_atr14
_feature_cols_for_b2 = train_mod._feature_cols_for_b2
_map_labels = train_mod._map_labels
_build_pipeline = train_mod._build_pipeline
apply_feature_set = train_mod.apply_feature_set
...
def run_cv(
    ...
    # aplica feature set seleccionado (subset si procede)
    ...
    if recommended is not None:
        ...
        logger.info(
            ...
            recommended_by=str(reason),
            ev_mean_at_thr=float(ev_stats.get("mean", np.nan)),
            folds_covered=int(ev_stats.get("folds_covered", 0)),
        )
    ...
    if metrics_mean:
        log_metrics({f"mean_{k}": v for k, v in metrics_mean.items() if v is not None})
    if metrics_std:
        log_metrics({f"std_{k}": v for k, v in metrics_std.items() if v is not None})
    if recommended is not None:
        log_params({"recommended_threshold": recommended, "recommended_by": reason})
        logger.info(
            "threshold_updated",
...
def main():
    ...
    global DAYS_HISTORY, DATE_FROM, DATE_TO, THRESHOLDS
    ...
    ap.add_argument(
        "--thresholds", type=str, default=None, help="Lista (JSON o CSV) de thresholds para la grid"
    )
    ...
    try:
        ...
        if _sel_path.exists():
            _sel = _json.loads(_sel_path.read_text(encoding="utf-8"))
            ...
            def _set_if_missing(attr, key, *, dump_json=False):
                ...
                if cur in (None, ""):
                    ...
                    if val is not None:
                        setattr(args, attr, _json.dumps(val) if dump_json else val)
