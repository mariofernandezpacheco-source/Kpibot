...
# ---------- DAT ----------
def dat_update(args):
    script = ROOT / "DAT_Data_download.py"
    if not script.exists():
        print(
            "⚠️  No encuentro DAT_Data_download.py. Ajusta el nombre en pibot_cli.py si tu script se llama distinto."
        )
        return
    opts = []
    if args.timeframe:
...
def rsh_cv(args):
    ...
    if not script.exists():
        print("⚠️  No encuentro RSH_TimeSeriesCV.py")
        return
    if args.ticker:
...
def rsh_select(args):
    ...
    if not script.exists():
        print("⚠️  No encuentro RSH_Scenarios.py")
        return
    if args.ticker:
...
# ---------- TRN ----------
def trn_fit(args):
    script = ROOT / "TRN_Train.py"
    if not script.exists():
        print("⚠️  No encuentro TRN_Train.py")
        return
    opts = []
    if args.ticker:
...
# ---------- LIV ----------
def liv_paper(args):
    script = ROOT / "LIV_PaperWorker.py"
    if not script.exists():
        print("⚠️  No encuentro LIV_PaperWorker.py")
        return
    opts = ["--mode", "paper"]
